...
for _, row in df.iterrows():
    if isinstance(row['id'], str) and row['id'].startswith('d'):
        relojes_data[row['id']] = {
            'nombre': row['nombre'],
            'precio': f"MX$ {int(row['precio'])}",
            'enlace': f"dama html/{row['id']}.html"  # Ruta para dama
        }

# Leer HTML
with open('dama.html', 'r', encoding='utf-8') as file:
    soup = BeautifulSoup(file, 'html.parser')

# Actualizar elementos
for i in range(1, 33):  # Asumiendo que hay hasta d40
    reloj_id = f'd{i}'
...
# Guardar cambios
with open('dama.html', 'w', encoding='utf-8') as file:
    file.write(str(soup))

print("Actualización completada: dama.html")
